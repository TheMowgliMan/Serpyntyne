...
{
    klog(logstatus, "uACPI Kernel API: ");
...
void *uacpi_kernel_map(uacpi_phys_addr addr, uacpi_size len)
{
    ...
    uacpi_klog(LOG_INFO, "Mapping addr %x, offset %d, to floor %x and cieling %x\r\n",
               (int64_t)addr, (int64_t)len, (int64_t)floor, (int64_t)cieling);
